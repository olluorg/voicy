...
окно при этом не закроется, пока его не закроют. Перед этим он проверит, не вышла
ли новая версия, и предложит обновиться. Второе окно второй voicy не запускает,
а открывает консоль первого: сервер и установка на машине — по одному (точнее,
на кэш `VOICY_CACHE`); команды-клиенты вроде `say` и `hear` это не ограничивает.

Обновиться из командной строки — `voicy update` (`--check` — только узнать,
`--yes` — не спрашивать): бинарник скачивает последний выпуск с GitHub,
сверяет его с контрольной суммой выпуска и встаёт на своё место. Собранный
из исходников так не обновляется — ему git pull.
...
`setup` качает готовые библиотеки и модели (7.8 ГБ). Сначала он показывает, что
и в какой каталог ляжет и сколько весит, и ждёт согласия; без терминала — только
с `--yes`. Каталог — `~/.cache/voicy` (на Windows `%LOCALAPPDATA%\voicy`),
меняется через `VOICY_CACHE`. Оборванную закачку `setup` повторяет сам и продолжает
с места обрыва, в том числе при следующем запуске. Сервер при старте сверяет кэш
со списком файлов и неполную установку достраивает сам: то, что уже скачано, —
молча, а то, что надо скачать, — только с согласия (без терминала — называет,
чего нет, и как докачать). Обёртку над C++-интерфейсом

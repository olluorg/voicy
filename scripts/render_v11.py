...
OUT = pathlib.Path(os.environ.get("OUTDIR","v7")); OUT.mkdir(exist_ok=True)
V = set("аеёиоуыэюя")
...
REF = os.environ.get("REF", "ys")
ref_a, ref_t = preprocess_ref_audio_text(f"lv/ref_{REF}.wav",
                                         acc.process_all(next(v for k, v in _rt.items() if f"ref_{REF}" in k)))
print(f"референс: {REF}", flush=True)

doc = json.load(open(os.environ.get("CUES","cues.json"), encoding="utf-8"))
...
for name, base in (("solid", solid), ("joined", joined)):
    for f in [float(x) for x in os.environ.get("FACTORS","1.0,0.78,0.72,0.66").split(",")]:
        y = stretch(base, f)
        tag = f"{REF}-{name}-{f:.2f}"
...
json.dump({"total_syllables": TOTAL_SYL, "results": results},
          open(os.environ.get("RESULTS","results_v7.json"), "w", encoding="utf-8"), ensure_ascii=False, indent=1)

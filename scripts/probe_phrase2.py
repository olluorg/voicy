...
OUT = "probe2"; os.makedirs(OUT, exist_ok=True)
...
slow = json.load(open("lv/turgenev_texts.json", encoding="utf-8"))
old = json.load(open("lv/ref_texts.json", encoding="utf-8"))
refs = {}
for k, v in slow.items():
    refs["Тургенев " + os.path.basename(k).replace("ref_", "").replace(".wav", "")] = (k.replace("\\", "/"), v)
for k, v in old.items():
    b = os.path.basename(k)
    if b == "ref_cdg.wav":
        refs["Тургенев старый"] = (k.replace("\\", "/"), v)
    if b == "ref_swn.wav":
        refs["Белые ночи"] = (k.replace("\\", "/"), v)
...
json.dump(rows, open("results_probe2.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)

...
def render(scale: float, pause_mode: str, tag: str, pause_scale: float = 1.0):
    parts, t0 = [], time.perf_counter()
    for cue in doc["cues"]:
        if cue["kind"] == "beat":
            parts.append(("gap", int(cue["ms"] * pause_scale)))
            continue
        st = STYLE[cue["kind"]]
        ...
        if pause_mode != "hard":
            w = fade(w, sr, FADE_MS)
        parts.append(("gap", int(st["pad_before"] * pause_scale)))
        parts.append(("aud", w))
        parts.append(("gap", int(st["pad_after"] * pause_scale)))
...
# C. reach the target rate by pausing more, not by drawling
for ps in (2.0, 3.0):
    render(1.0, "tone", f"pauses-x{ps}", pause_scale=ps)

json.dump({"total_syllables": TOTAL_SYL, "results": results},
          open("results_pauses.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)

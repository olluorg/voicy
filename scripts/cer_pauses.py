...
d=json.load(open("results_pauses.json",encoding="utf-8"))
for r in d["results"]:
    segs,_=m.transcribe(f"v2/{r['file']}",language="ru",beam_size=5)
    hyp=" ".join(s.text for s in segs)
    r["cer"]=round(cer(norm(ref),norm(hyp))*100,1)
    print(f"{r['tag']:14} {r['syl_per_s']:.2f} слог/с ~{r['wpm']:.0f} слов/мин  CER {r['cer']:5.1f}%",flush=True)
json.dump(d,open("results_pauses.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)

...
d=json.load(open("results_v3.json",encoding="utf-8"))
for r in d["results"]:
    segs,_=m.transcribe(f"v3/{r['tag']}.wav",language="ru",beam_size=5)
    hyp=" ".join(s.text for s in segs)
    r["cer"]=round(cer(norm(ref),norm(hyp))*100,1)
    print(f"{r['tag']:10} ~{r['wpm']:3} слов/мин  тишины {r['silence_pct']:4.1f}%  CER {r['cer']:5.1f}%",flush=True)
json.dump(d,open("results_v3.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)
